...
#include <gmapping_fix/scanmatcher/scanmatcher.h>
...
*/
void ScanMatcher::computeActivePoint(ScanMatcherMap& map, double ActivePoint_x_meter, double ActivePoint_y_meter)
{
	if (m_activeAreaComputed)
	{ return; }
	
	Point p;		//p = position; meter
	p.x = ActivePoint_x_meter;
	p.y = ActivePoint_y_meter;
	IntPoint p0=map.world2map(p);

	// check map size -> build a bigest map that fix the all the point
	...
	//printf("[%.2f, %.2f], <%d, %d>\n",p.x, p.y, p0.x, p0.y);
	HierarchicalArray2D<PointAccumulator>::PointSet activeArea;
	IntPoint cp = map.storage().patchIndexes(p0);
	activeArea.insert(cp);
...
double ScanMatcher::registerPoint(ScanMatcherMap& map, double registerPoint_x_meter, double registerPoint_y_meter, bool is_occupied)
{
	if (!m_activeAreaComputed)
	{ computeActivePoint(map, registerPoint_x_meter, registerPoint_y_meter); }
	
	//this operation replicates the cells that will be changed in the registration operation
	map.storage().allocActiveArea();
	
	Point p;		//p = position; meter
	p.x = registerPoint_x_meter;
	p.y = registerPoint_y_meter;
	IntPoint cp = map.world2map(p);	//cp = position; pixel

	//printf("[%.2f, %.2f], <%d, %d>\n",p.x, p.y, cp.x, cp.y);
	if(is_occupied) //black area
	{ map.cell(cp).update(true, p); }
	else		//empty area: gray area
	{ map.cell(cp).update(false, Point(0,0)); }
